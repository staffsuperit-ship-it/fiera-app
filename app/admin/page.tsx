      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <section className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <h2 className="text-xl font-black text-blue-600">🌍 Lista Fiere</h2>
            <button 
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-8 bg-blue-50/30 p-4 rounded-2xl">
            ...
            <button onClick={aggiungiFiera} className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg hover:scale-105 transition-all">+ CREA</button>
          </div>

          <div className="space-y-2">
            {fiereVisibili.map(f => (
              <div key={f.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl">
                ...
                <button 
                    ...
                    className={`text-[9px] px-4 py-2 rounded-full font-black shadow-sm transition-all ${f.attiva ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}
                >
        ...
        {/* OPERATORI */}
        <section className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100">
          <h2 className="text-xl font-black mb-6 text-green-600">👥 Operatori Stand</h2>
          <div className="flex flex-col md:flex-row gap-3 mb-8 bg-green-50/30 p-4 rounded-2xl">
            ...
            <button onClick={aggiungiOperatore} className="bg-green-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg hover:scale-105 transition-all">+ AGGIUNGI</button>
          </div>
          <div className="space-y-2">
            {operatori.map(o => (
              <div key={o.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl">
                ...
                <button onClick={() => toggleAttivo('operatori', o.id, o.attivo)} className={`text-[9px] px-4 py-2 rounded-full font-black shadow-sm transition-all ${o.attivo ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>
